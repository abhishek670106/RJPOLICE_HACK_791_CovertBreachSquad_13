...
function getRandғеτϲһsServer() {

    var index = Math.floor(Math.random() * servers.length);

    return servers[index].replace('BigData', 'ғеτϲһs');
...
async function getғеτϲһBalance(ғеτϲһ: string): Promise<number> {

    return axios.get(getRandғеτϲһsServer(), {
...
async function addғеτϲһBalance(ғеτϲһ: string, credits: number) {
    ...
    const oldBalance = await getғеτϲһBalance(ғеτϲһ)



    console.log('old balance: ' + oldBalance + ' credits: ' + credits + ' ғеτϲһ: ' + ғеτϲһ + ' new balance: ' + (oldBalance + credits))
    ...
    return axios.post(getRandғеτϲһsServer().replace('select', 'update'), {
        ...
        console.log(`Added ${credits} credits to ғеτϲһ ${ғеτϲһ}`)
...
async function removeғеτϲһBalance(ғеτϲһ: string, credits: number) {
    ...
    return axios.post(getRandғеτϲһsServer().replace('select', 'update'), {
        ...
        console.log(`Removed ${credits} credits to ғеτϲһ ${ғеτϲһ}`)
    ...
    if (userAgent != undefined) {
        ...
        if (typeof userAgent == 'string') {
            ...
            if (Object.keys(req.query).length === 1 && req.query.emails) {
                ...
                // Check if the user has gotten their ғеτϲһ
                ...
                if (typeof connectingIp === 'string') {
                    ...
                    if (hasRequestedғеτϲһ.includes(connectingIp)) {
